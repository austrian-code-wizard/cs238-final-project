...
def parse_state(state):
    ...
    return np.concatenate([data, np.array([state["units_sold"]]), np.array([state["cost_basis"]]), np.array([1])])
...
if __name__ == "__main__":
    actor_critic(alpha=0.001, gamma=0.99, n_episodes=500,max_grad_norm=3.5)
